...
export default function EditKontenPage() {
  ...
  const [registerKonten, setRegisterKonten] = useState({
    ...
    subHeader:
      "Silahkan daftar untuk membuat akun di sistem layanan pertanahan kantah Gowa.",
  ...
  const [alurSistem, setAlurSistem] = useState([
    ...
    {
      id: 2,
      judul: "2. Ajukan Permohonan",
      deskripsi:
        "Setelah akun disetujui, ajukan permohonan layanan dengan mengisi form yang tersedia.",
    },
    {
      id: 3,
      judul: "3. Pantau Status",
      deskripsi:
        "Pantau perkembangan permohonan dan terima notifikasi melalui sistem dan email.",
    },
  ]);
  ...
  const [fiturUtama, setFiturUtama] = useState([
    {
      ...
      deskripsi:
        "Ajukan dan pantau status permohonan layanan pertanahan secara real-time",
    },
    {
      id: 2,
      judul: "Sistem Verifikasi",
      deskripsi:
        "Proses verifikasi akun dan permohonan yang aman dan terstruktur",
    },
    {
      id: 3,
    ...
    },
    {
      ...
      deskripsi:
        "Terima pemberitahuan melalui sistem dan email untuk setiap update",
    },
  ]);
  ...
  // //logika maskot login
  // const handleLoginConfigMaskotChange = (
  //   e: React.ChangeEvent<HTMLInputElement>
  // ) => {
  //   if (e.target.files && e.target.files[0]) {
  //     setLoginConfigMaskotFile(e.target.files[0]);
  //   }
  // };

  //fungsi navbar icon

  const [selectedNavbarIcon, setSelectedNavbarIcon] = useState<File | null>(
    null
  );
  ...
  //logika register config
 const [registerConfigKonten, setRegisterConfigKonten]=useState<{
  headerTitle: string;
  subHeader: string;
  listJabatan: string[];
  maskotFile: any;
  ...
  const registerConfigMaskotInputRef = useRef <HTMLInputElement>(null);
  const registerConfigBgInputRef = useRef <HTMLInputElement>(null);

    const handleRegisterConfigBgClick = () => {
      registerConfigBgInputRef.current?.click();
    };
    const handleMaskotRegisterConfigChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      const file = e.target.files?.[0];
      if (file){
        setRegisterConfigMaskotFile(file);
      }
    };
    const handleRegisterConfigBgChange = (e: React.ChangeEvent<HTMLInputElement>)=> {
      const file = e.target.files?.[0];
      if (file){
        setRegisterConfigBgFile(file);
      }
    }

const [registerConfigBgFile, setRegisterConfigBgFile] = useState<File | null>(null);
const [registerConfigMaskotFile, setRegisterConfigMaskotFile] = useState<File | null>(null);
  const handleSaveRegisterConfig = async () => {
    try {
      ...
      formData.append('opsi_jabatan', JSON.stringify(registerConfigKonten.listJabatan));

      if(registerConfigMaskotFile){
        formData.append('maskot', registerConfigMaskotFile);
      } 
      if (registerConfigBgFile){
...
useEffect(() => {
  const loadExistingConfig = async () => {
    ...
    if (data) {
      setRegisterConfigKonten({
        headerTitle: data.judul_utama || "",
        subHeader: data.sub_judul || "",
        listJabatan: typeof data.opsi_jabatan === 'string'
        ? JSON.parse(data.opsi_jabatan)
        : (data.opsi_jabatan || []),
        maskotFile: null,
        backgroundFile: null
      });
  ...
  const heroMaskotInputRef = useRef<HTMLInputElement>(null);;
  const heroBgInputRef = useRef<HTMLInputElement>(null);;

  const [heroMaskotFile, setHeroMaskotFile] = useState<File | null>(null);
  const [heroBgFile, setHeroBgFile] = useState<File | null>(null);


  const handleHeroMaskotChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file){setHeroMaskotFile(file);
    }};

  
  const handleHeroBgChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file){setHeroBgFile(file);
    }};
  ...
  const handleSave = async () => {
    try {
      ...
      if (heroMaskotFile) { // Ini Maskot
        formData.append("maskot", heroMaskotFile);
      }
    ...
    const fetchContentData = async () => {
      try {
        ...
        setKonten({
          ...
          heroTitle2:
            data.heroTitle2 || "Sistem Informasi Kantor Pertanahan Gowa",
          heroTitle3:
            data.heroTitle3 ||
            "Platform digital untuk Notaris dan PPAT dalam melakukan pendaftaran, pengajuan permohonan, dan pemantauan status layanan pertanahan secara efisien dan terpadu.",
          footerText1:
  ...
  //logika loginconfig
  const loginConfigMaskotInputRef = useRef <HTMLInputElement>(null);
  const loginConfigBgInputRef = useRef <HTMLInputElement>(null);


  const handleMaskotLoginConfigChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file){
      setLoginConfigMaskotFile(file);
    }};

    const handleLoginConfigBgChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      const selectedFile = e.target.files?.[0]; // ambil file ke variabel lokal
      if (selectedFile) {
        setLoginConfigBgFile(selectedFile);
      }
    };

    const handleLoginConfigBgClick = () => {
      loginConfigBgInputRef.current?.click();
    };

const [loginConfigBgFile, setLoginConfigBgFile] = useState<File | null>(null);
const [loginConfigMaskotFile, setLoginConfigMaskotFile] = useState<File | null>(null);
  ...
  const handleSaveLoginConfig = async () => {
    try {
      ...
      if (loginConfigMaskotFile) {
        formData.append("maskot", loginConfigMaskotFile);
      }
      if (loginConfigBgFile) {
      ...
      setLoginConfigMaskotFile(null);
      setLoginConfigBgFile(null);
    } catch (error) {
      console.error("Gagal simpan:", error);
      alert(
        `Gagal menyimpan data:' ${
  ...
  const handleIconChange = (id: any, file: File) => {
    setFiturUtama((prev: any) =>
      prev.map((f: any) => (f.id === id ? { ...f, newIconFile: file } : f))
    );
  };
                ...
                <section className="bg-white rounded-[25px] shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  <div className="p-6 space-y-6">
                    <div className="space-y-1">
                      <label className="text-[15px] font-bold text-gray-900 ml-1">
                        Header Utama
                      </label>
                      <input
                        type="text"
                        ...
                        onChange={(e) =>
                          setKonten({ ...konten, heroTitle1: e.target.value })
                        }
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setKonten({ ...konten, heroTitle2: e.target.value })
                        }
                        className="w-full px-4 py-3 text-gray-700 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-[16px] font-semibold"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setKonten({ ...konten, heroTitle3: e.target.value })
                        }
                    ...
                    <div className="space-y-4 pt-6 border-t border-gray-200">
                      <h4 className="text-[16px] font-bold text-gray-900 border-b pb-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                          ...
                          >
                            {heroBgFile ? (
                              <div className="relative w-full h-full flex flex-col align-items-xxl-center">
                                <img
                                  src={URL.createObjectURL(heroBgFile)}
                                  alt="Preview Background"
                                  className="w-full h-24 object-cover rounded-lg mb-2 shadow-md"
                                />
  ...
  {heroBgFile 
    ...
    : "Klik untuk ganti background kantor"}
                  ...
                  <div className="p-6 space-y-5">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setKonten({ ...konten, footerText1: e.target.value })
                        }
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setKonten({ ...konten, footerText2: e.target.value })
                        }
                        className="w-full px-4 py-3 text-gray-700 bg-gray-50 border-2 border-gray-200 rounded-xl text-[16px] font-semibold focus:border-[#56b35a] focus:outline-none transition"
                      />
                    </div>
                  ...
                  <div className="p-6 space-y-6">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setKonten({ ...konten, navText1: e.target.value })
                        }
                        className="w-full px-4 py-3 text-gray-700 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-[16px] font-semibold"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setKonten({ ...konten, navText2: e.target.value })
                        }
                        className="w-full px-4 py-3 text-gray-700 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-[16px] font-semibold"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      <label className="text-[15px] font-bold text-gray-900 ml-1 block mb-3">
                        Icon
                      </label>
              ...
              {activeTab === "alur" && (
                <section className="bg-white rounded-[25px] shadow-sm border border-gray-200 overflow-hidden">
                  <div className="bg-[#8b5e3c] p-4 px-6 text-white font-bold text-[16px] flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <ListOrdered size={18} /> Alur Penggunaan Sistem
                    </div>
                    <button
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {alurSistem.map((step) => (
                        <div
                          key={step.id}
                          className="p-5 bg-gray-100 rounded-[20px] border border-gray-200 space-y-4 relative"
                        >
                          <div className="space-y-1">
                            <label className="text-[15px] font-bold text-gray-900 ml-1">
                              Judul Langkah
                            ...
                            <input
                              ...
                              value={step.judul}
                              onChange={(e) =>
                                updateAlur(step.id, "judul", e.target.value)
                              }
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                updateAlur(step.id, "deskripsi", e.target.value)
                              }
                          ...
                          <div className="pt-2">
                            <label className="group w-full h-8 bg-white rounded-lg border border-dashed border-gray-300 flex items-center justify-center text-[10px] text-gray-900 font-bold hover:bg-green-50/50 hover:border-[#56b35a] cursor-pointer transition-all duration-300">
                              <ImageIcon
                                size={12}
                                className="mr-2 group-hover:text-[#56b35a] transition-colors"
                              />
                              <span className="group-hover:text-[#56b35a] transition-colors">
                                {(step as any).newIconFile
                                  ? (step as any).newIconFile.name
                                  : "Ganti Icon Langkah"}
                              </span>
                              <input
                                type="file"
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              value={fitur.judul}
                              onChange={(e) =>
                                updateFitur(fitur.id, "judul", e.target.value)
                              }
    ...
    {/* gabisa diubah */}
                    <div className="space-y-4 pt-6 border-t border-gray-100">
                      <h4 className="text-[16px] font-bold text-gray-900 border-b pb-2">
                        Konten di Dalam Kotak Login (ReadOnly)
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="space-y-2">
                          <label className="text-[16px] font-bold text-gray-700 ml-1">
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setLoginKonten({
                                ...loginKonten,
                                placeholderPassword: e.target.value,
                              })
                            }
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-gray-900"
                          />
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setLoginKonten({
                                ...loginKonten,
                                textLupaPassword: e.target.value,
                              })
                            }
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-gray-900"
                          />
                    ...
                    <div className="space-y-4 pt-6 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <input
                            type="file"
                            ref={loginConfigBgInputRef}
                            onChange={handleLoginConfigBgChange}
                            accept="image/*"
                            className="hidden"
                          />

                          <div
                            onClick={handleLoginConfigBgClick}
                            className="group relative overflow-hidden rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 p-4 flex flex-col items-center justify-center min-h-[150px] hover:border-[#56b35a] hover:bg-green-50/30 cursor-pointer transition-all duration-300"
                          >
                            {loginConfigBgFile ? (
                              <div className="relative w-full h-full flex flex-col align-items-xxl-center">
                                <img
                                  src={URL.createObjectURL(loginConfigBgFile)}
                                  alt="Preview Background"
                                  className="w-full h-24 object-cover rounded-lg mb-2 shadow-md"
                                />
                                <p className="text-[11px] text-[#56b35a] font-bold">
                                  {loginConfigBgFile?.name}{" "}
                                </p>
                              </div>
                            ) : (
                              <>
...
<p className="text-[12px] font-bold text-gray-700">
  {loginConfigBgFile 
    ? `File: ${(loginConfigBgFile as File).name}` 
    : "Klik untuk ganti background kantor"}
</p>
                              </>
                            )}
              ...
              {activeTab === "register" && (
                <section className="bg-white rounded-[25px] shadow-sm border border-gray-200 overflow-hidden mb-20">
                  <div className="bg-[#8b5e3c] p-4 px-6 text-white font-bold text-[16px] flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <UserPlus size={18} /> Pengaturan Tampilan Halaman
                      Register
                    </div>
                    <button
                      onClick={handleSaveRegisterConfig} 
                      className="flex items-center gap-2 bg-[#56b35a] hover:bg-[#469e4a] text-white px-4 py-1.5 rounded-lg font-bold transition text-[15px] shadow-sm"
                    >
                      <Save size={14} /> Simpan
                    </button>
                  ...
                  <div className="p-6 space-y-10">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input
                            type="text"
                            value={registerConfigKonten.headerTitle}
                            onChange={(e) =>
                              setRegisterConfigKonten({
                                ...registerConfigKonten,
                                headerTitle: e.target.value,
                              })
                            }
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-gray-900"
                        ...
                        <div className="space-y-2">
                          ...
                          <textarea
                            rows={2}
                            value={registerConfigKonten.subHeader}
                            onChange={(e) =>
                              setRegisterConfigKonten({
                                ...registerConfigKonten,
                                subHeader: e.target.value,
                    ...
                    <div className="space-y-4 pt-6 border-t border-gray-100">
                      <h4 className="text-[16px] font-bold text-gray-900 border-b pb-2">
                        Konten di Dalam Kotak Register (ReadOnly)
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="space-y-2">
                          <label className="text-[16px] font-bold text-gray-700 ml-1">
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setRegisterKonten({
                                ...registerKonten,
                                placeholderNama: e.target.value,
                              })
                            }
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-gray-900"
                          />
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setRegisterKonten({
                                ...registerKonten,
                                placeholderNoHp: e.target.value,
                              })
                            }
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-gray-900"
                          />
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setRegisterKonten({
                                ...registerKonten,
                                labelKonfirmasi: e.target.value,
                              })
                            }
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-gray-900"
                          />
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setRegisterKonten({
                                ...registerKonten,
                                placeholderKonfirmasi: e.target.value,
                              })
                            }
                            className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:border-[#56b35a] focus:outline-none transition text-gray-900"
                          />
                      ...
                      <div className="space-y-4 pt-6 border-t border-gray-100">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-3">
                            ...
                            <input
  type="file"
  ref={registerConfigBgInputRef}
  onChange={handleRegisterConfigBgChange}
  className="hidden"
  accept="image/*"
/>
                            <div 
                            onClick={handleRegisterConfigBgClick}
                            className="group relative overflow-hidden rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 p-4 flex flex-col items-center justify-center min-h-[150px] hover:border-[#56b35a] hover:bg-green-50/30 cursor-pointer transition-all duration-300">
                             { registerConfigBgFile ? (
                              <p className="text-[#56b35a] font-bold text-sm">File terpilih: {registerConfigBgFile.name}</p>
                             ): (
                              <>
                               <Monitor
                                className="text-gray-900 mb-2 group-hover:text-[#56b35a] transition-colors"
                    ...
                    <div className="space-y-4 pt-6 border-t border-gray-100">
                      <div className="flex items-center justify-between border-b pb-2">
                        <h4 className="text-[16px] font-bold text-gray-900 border-b pb-2">
                          Opsi Jabatan (Dropdown)
                        </h4>
                        <span className="text-[12px] text-gray-500 font-medium italic">
                          *Input ini akan muncul di pilihan dropdown register
                        </span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {registerConfigKonten.listJabatan.map((jab, index) => (
                          <div
                            key={index}
                          ...
                          >
                            <div className="flex-1 relative">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  const newList = [
                                    ...registerConfigKonten.listJabatan,
                                  ];
                            ...
                            type="button"
                              onClick={() => {
                                const newList =
                                  registerConfigKonten.listJabatan.filter(
                                    (_, i) => i !== index
                                  );
                                setRegisterConfigKonten({
                                  ...registerConfigKonten,
          ...
          <footer className="w-full bg-[#1a1a1a] text-white py-6 text-center mt-10">
            ...
            <p className="text-[9px] opacity-50 tracking-widest mt-1">
              {konten.footerText2}
            </p>
          </footer>
        </main>
